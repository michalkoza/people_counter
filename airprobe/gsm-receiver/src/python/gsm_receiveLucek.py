...
class tuner(gr.feval_dd):
    ...
    def eval(self, freq_offet):
        ...
        print("=======setting freq==========="+str(freq_offet))
        return freq_offet
...
class gsm_receiver_first_blood(gr.top_block):
    ...
    def _set_source_file(self):
        nazwa_pliku = self.options.inputfile
        zrodlo = gr.file_source(gr.sizeof_gr_complex, nazwa_pliku, False)
        return zrodlo

#    def _set_source_usrp(self):
#        nazwa_pliku = self.options.inputfile
#        zrodlo = gr.file_source(gr.sizeof_gr_complex, nazwa_pliku, False)
#        return zrodlo

    def _set_source_uhd(self):
        zrodlo = uhd.usrp_source(
            device_addr="",
        ...
        zrodlo.set_samp_rate(self.input_rate)
        zrodlo.set_center_freq(self.freq)
        zrodlo.set_gain(self.options.gain)
        return zrodlo
...
#        print "input_rate:", self.input_rate, "sample rate:", self.sps, " filter_cutoff:", filter_cutoff, " filter_t_width:", filter_t_width
	...
	parser.add_option("-g", "--gain", type="int", default=30,
                          help="gain")
	parser.add_option("-a", "--arf", type="int", default=1,
                          help="arf channel")
        ...
        parser.add_option("-k", "--key", type="string", default="00 00 00 00 00 00 00 00",
                          help="KC session key")
        parser.add_option("-c", "--configuration", type="string", default="",
                          help="Decoder configuration")
        parser.add_option("-m", "--multiply", type="int", default=1,
                          help="Signal multiply factor")
